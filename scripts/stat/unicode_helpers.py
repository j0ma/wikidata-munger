...
def maybe_strip(word: str, strip: bool = False) -> str:
    return str(word).strip() if strip else str(word)


@lru_cache(maxsize=CACHE_MAX_SIZE)
def unicode_blocks(
    word: str, strip: bool = False, ignore_punctuation: bool = False
) -> Counter:
    punctuation_cond = (
        lambda w: not is_punctuation(str(w)) if ignore_punctuation else True
    )

    return Counter(
        blocks.of(c)

        for c in maybe_strip(word, strip)
...
@lru_cache(maxsize=CACHE_MAX_SIZE)
def most_common_unicode_block(
    word: str, strip: bool = False, ignore_punctuation: str = False
) -> str:
    return unicode_blocks(
        word, strip=strip, ignore_punctuation=ignore_punctuation
    ).most_common(1)[0][0]
